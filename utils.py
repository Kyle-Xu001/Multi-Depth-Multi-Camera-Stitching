...
def featureHeatmap(image, features):
    ...
    for feature in features:
        if (feature[0] > 11 and feature[0] < image.shape[1]-11) and (feature[1] > 11 and feature[1] < image.shape[0]-11):
            featImg[int(feature[1])-offset:int(feature[1])+offset+1,int(feature[0])-offset:int(feature[0])+offset+1] += 3500*guassian_pitch
...
def guassian(dsize, sigma):
    ...
    for i in range(dsize):
        for j in range(dsize):
            guassian_pitch[i,j] = math.exp((-1/(2*sigma**2))*(np.square(i-offset) + np.square(j-offset)))/(2*math.pi*sigma**2)
